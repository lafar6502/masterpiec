...
void loggingTask() {
  ...
  if (t - lastRun > 5L * 60 * 1000) {
    lastRun = t;
    uint8_t d = RTC.dow - 1;
    g_DailyLogEntries[d].FeederTotalSec += (uint16_t) (g_FeederRunTime / 1000L);
    g_FeederRunTime = 0;
    g_DailyLogEntries[d].FeederTotalSec += (uint16_t) (g_P1Time / 1000L);
    g_P1Time = 0;
    g_DailyLogEntries[d].FeederTotalSec += (uint16_t) (g_P2Time / 1000L);
    g_P2Time = 0;
